...
module.exports = function(ast, options) {
  ...
  var PRIVATE_VAR_PREFIX = '__p_';

  var INPUT_VAR = 'input',
      POS_VAR = PRIVATE_VAR_PREFIX + 'pos',
      PREV_POS_VAR = PRIVATE_VAR_PREFIX + 'ppos';
    ...
    var name,
        ...
        sources = {
          grammar: [
            ...
            '  var '+INPUT_VAR+';',
            '  ',
            '  var '+POS_VAR+', // 0, parser position',
            '      '+PREV_POS_VAR+'; // 0, previous parser position',
            ...
            '        function offset() { return '+POS_VAR+'; }',
            '        function text() { return \'foo\'; }; ',
            '      #end',
            '      ',
            '      #if initializerDef',
            '        ',
            ...
            '      if ('+POS_VAR+' > rmfpos) rmfpos = '+POS_VAR+';',
            '      var e = failures['+POS_VAR+'] ||',
            '             (failures['+POS_VAR+'] = merr(found));',
            ...
            '      function _ckey(name) { return name+"@"+'+POS_VAR+'; } // get cache key',
            '      ',
            ...
            '    ',
            '    // get current char',
            '    function cc() { return ('+POS_VAR+' < ilen) ? quote('+INPUT_VAR+'.charAt('+POS_VAR+')) : EOI; }',
            '    ',
            // rule_ref ==========
            '    #if stats.rule_ref',
            ...
            '        return {',
            '          pos: pp, end: '+POS_VAR+',',
            '          match: '+INPUT_VAR+'.substring(pp, '+POS_VAR+')',
            '        }',
            '      }',
            '      ',
            '      function action(f, code) {',
            '        return inctx(function() {',
            '          '+PREV_POS_VAR+' = '+POS_VAR+'; var res; // previous pos',
            '          f(); res = code(cctx, chunk('+PREV_POS_VAR+'));',
            '          if (res === null) { '+POS_VAR+' = '+PREV_POS_VAR+';',
            ...
            '    #if stats.sequence',
            '      function seqnc(/*f...*/) {',
            '        '+PREV_POS_VAR+' = '+POS_VAR+', // previous position',
            '        var fs = arguments,',
            '            s = [],',
            '            on_miss = function(e) {',
            '                        '+POS_VAR+' = '+PREV_POS_VAR+'; throw e; };',
            ...
            '      function match(str) {',
            '        var slen = str.length;',
            '        if (('+POS_VAR+' + slen) > ilen) {',
            '          failed(quote(str), EOI); // exits',
            '        }',
            '        if ('+INPUT_VAR+'.substr('+POS_VAR+', slen) === str) {',
            '          '+POS_VAR+' += slen;',
            '          return str;',
            '        }',
            '        failed(quote(str), cc());',
            ...
            '    #if stats.simple_and',
            '      function and(f) {',
            '        '+PREV_POS_VAR+' = '+POS_VAR+'; var missed = 0;',
            ...
            '        if (res = rx.exec('+INPUT_VAR+'.substr('+POS_VAR+'))) {',
            '           if (res.index !== 0) failed(desc, cc());',
            '           '+POS_VAR+' += res[0].length;',
            '           return res[0];',
            '        } else failed(desc, cc());',
            '      }',
            '      re = def(re);',
            ...
            '    #if stats.any',
            '      function ch() { // char',
            '        if ('+POS_VAR+' >= ilen) failed(ANY, EOI);',
            '        return '+INPUT_VAR+'['+POS_VAR+'++];',
            '      }',
            '      ch = def(ch);',
            '      ',
            '    #end',
            ...
            '        '+POS_VAR+' = 0, ilen = _input.length, '+INPUT_VAR+' = _input;',
