    ...
    var name,
        ...
        sources = {
          grammar: [
            ...
            //'          if ((startRule != "start") &&',
            '          if (#{allowedStartRules}.indexOf(startRule) < 0) {',
            '            throw new Error("Rule " + quote(startRule) + " is not in the list of allowed start rules.");',
            '          }',
            '        #else',
            '          #if firstRule',
            '            if ((startRule != "start") &&',
  ...
  var emit = buildNodeVisitor({
    grammar: function(node) {
      ...
      return fill("grammar", {
        ...
        allowedStartRules: allowedStartRules //&& allowedStartRules.length
                           ? ('["' + allowedStartRules.join('","') + '"]')
                           : null,
        blocks:         node.blocks,
        blocksDef:      blocksDef,
        stats:          node.stats
