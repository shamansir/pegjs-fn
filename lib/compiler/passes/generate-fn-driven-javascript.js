    ...
    var templates = {},
        sources = {
          grammar: [
            ...
            '      #else',
            '        var ref = def(function(f) { return f(); }); // just call passed function',
            '      #end',
            '      ',
            '    #end',
            // action ============
            ...
            '    ',
            '    #if usesContext',
            '      function prepare_ctx() { ctxl = -1; return ctx_lvl(); }',
            ...
            '        ',
            '        #if startRule',
            '          var startRule = #{OPTIONS}.startRule || #{string(startRule)};',
            '        #else',
            '          #if alwdStartRules',
            '            var startRule = #{OPTIONS}.startRule || #{string(firstAlwdStartRule)};',
            '          #else',
            '            #if firstRule',
            '              var startRule = #{OPTIONS}.startRule || #{string(firstRule)};',
            '            #else',
         ...
         // '      toSource: function() { return this._source; },',
            ...
            '  return __parser();',
            '  ',
            '})()' // parser-generator puts semicolon here itself
          ],
          rule: [
  ...
  var emit = buildNodeVisitor({
    grammar: function(node) {
      ...
      var stats = node.stats,
          usesContext = blocksDef || stats.action || stats.labeled || stats.rule_ref || stats.semantic_and || stats.semantic_not;
